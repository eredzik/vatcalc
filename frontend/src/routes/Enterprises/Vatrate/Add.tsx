...
export function VatrateAdd() {
    ...
    const formik = useFormik<{ vat_rate?: number, comment?: string }>({
        ...
        validationSchema: yup.object().shape({
            vat_rate: yup.number()
                ...
                .typeError("Musisz podać liczbę (należy używać kropki zamiast przecinka)")
        ...
        onSubmit: (async (v) => vatrateadd.mutate({
    ...
    return (
        <Layout>
            ...
            <Form onSubmit={formik.handleSubmit} error={vatrateadd.isError} success={vatrateadd.isSuccess}>
                <Form.Input
                    label="Stawka VAT"
                    id="vat_rate"
                ...
                <Message error content={"Błąd dodawania kontrahenta." + vatrateadd.error} />
                <Message success content={"Dodano stawkę VAT!"} />
                <Form.Group>
                    <Form.Button content="Dodaj" primary disabled={!formik.isValid} type="submit" />
                    <Form.Button content="Cofnij"
                        secondary type="button" onClick={() => history.goBack()} />
                </Form.Group>
            </Form>
        </Layout>
    )
