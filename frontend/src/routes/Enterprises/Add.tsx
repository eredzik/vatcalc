...
export default function EnterpriseAdd() {
  ...
  return (
    <Layout>
      ...
      />
      {activeStep === 0 && (
        <Page0 sendNipNumber={setNipNumber} nextPageHandler={nextPageHandler} />
      )}
      {activeStep === 1 && (
        <Page1 inputNipNumber={nipNumber} prevPageHandler={prevPageHandler} />
      )}
    </Layout>
  );
}
...
function Page0({ sendNipNumber, nextPageHandler }: Page0Props) {
  ...
  return (
    <Form onSubmit={formik.handleSubmit}>
      ...
      Najpierw wpisz numer nip swojej firmy - pobierzemy wszystkie możliwe
      informacje z bazy REGON. Możesz też wpisać dane ręcznie.
      <Form.Input
        label="Numer NIP"
        required
        fullWidth
        ...
        onChange={(e) => {
          console.log(formik.touched.nipNumber);
      ...
      <Grid item justify="center">
        <Button type="button" primary onClick={nextPageHandler}>
          Wpisz ręcznie
        </Button>
      </Grid>
    </Form>
...
function Page1({ inputNipNumber, prevPageHandler }: Page1Props) {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      ...
      name: yup
        .string()
        .test("nonempty", "Nazwa nie może być pusta", (val) =>
          val ? true : false
        ),
      taxSchema: yup
        .string()
        .test("unselected", "Wybierz formę opodatkowania", (val) => {
          console.log(val);
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Grid container alignItems="center" direction="column" spacing={1}>
        <Grid item>
          <Typography variant="h4">{steps[1]}</Typography>
        </Grid>
        <Grid>
          <TextField
            label="Numer NIP"
        ...
        <Grid item>
          ...
          >
            <Select
              ...
              onBlur={formik.handleBlur}
              error={
                formik.touched.taxSchema && Boolean(formik.errors.taxSchema)
              }
              // helperText={}
              id="taxSchema"
              name="taxSchema"
            >
              ...
              <MenuItem value="2">Podatek liniowy (19%)</MenuItem>
        ...
        <Grid item>
          <Button onClick={prevPageHandler} primary>
            Wstecz
          </Button>
        </Grid>
