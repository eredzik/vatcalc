...
export default function AddTradingPartner() {
  ...
  useEffect(() => {
    if (add_partner.status === "success") {
      history.push("trading_partner/");
    }
  }, [add_partner.status]);
  const { isSuccess, data, ...data_partner } = useREGONQueryByNIP(
    formik.values.nipNumber
  );
  useEffect(() => {
    if (isSuccess && data !== undefined) {
      console.log("Fired useeffect", isSuccess, data);
      formik.setFieldValue("fullName", data.company_name || "");
      formik.setFieldValue(
        "shortName",
        data.company_name.substring(0, 20) || ""
      );
  ...
  return (
    <Layout>
      ...
      >
          ...
          <Form.Input
            ...
            error={
              ...
              (data_partner.error?.error_code
                ? parse_error_code(data_partner.error?.error_code)
                : null)
            }
            required
            id="nipNumber"
        ...
        <Form.Group label="dupa" style={{ display: "flex", flexWrap: "wrap" }}>
          <Form.Input
            label="Kraj"
            required
            // id="country"
