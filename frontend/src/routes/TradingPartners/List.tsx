...
export default function TradingPartnersList() {
  ...
  const fav_enterprise_id =
    user.data?.data.fav_enterprise_id || enterprises.data?.data[0] || -1;
  ...
  const tradingPartnersList = useTradingPartnersList(
    1,
    fav_enterprise_id as number
  ); // always has to be present - eliminated by redirect above
  const columns = React.useMemo(
    () => [
      ...
      {
        Header: "Numer NIP",
        accessor: (r: TradingPartnerResponse) => r.nip_number,
      },
    ],
    []
  );
  const data = React.useMemo(
    () =>
      tradingPartnersList.data && tradingPartnersList.data.data.length > 0
        ...
        <Table.Body>
          {rows.map((row) => {
            ...
            return (
              <Table.Row>
                {row.cells.map((cell) => (
                  <Table.Cell>{cell.render("Cell")}</Table.Cell>
                ))}
                <Table.Cell>
                  {" "}
                  <Button
                    icon="cogs"
                    onClick={() =>
                      history.push(`/trading_partner/${row.original.id}`)
                    }
                  />
                </Table.Cell>
              </Table.Row>
